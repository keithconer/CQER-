...
-- ============================================================
-- START COPY: Budget Utilization Module
-- ============================================================
create table if not exists public.budget_utilizations (
  id uuid primary key default gen_random_uuid(),
  ...
  utilized_total numeric(14,2) not null default 0,
...
create index if not exists idx_budget_utilizations_created_by
on public.budget_utilizations (created_by);

create index if not exists idx_budget_utilizations_project_id
on public.budget_utilizations (project_id);

alter table public.budget_utilizations enable row level security;

drop policy if exists "Users can view own budget utilizations" on public.budget_utilizations;
create policy "Users can view own budget utilizations" on public.budget_utilizations
for select using (auth.uid() = created_by);

drop policy if exists "Users can create own budget utilizations" on public.budget_utilizations;
create policy "Users can create own budget utilizations" on public.budget_utilizations
for insert with check (auth.uid() = created_by);

drop policy if exists "Users can update own budget utilizations" on public.budget_utilizations;
create policy "Users can update own budget utilizations" on public.budget_utilizations
for update using (auth.uid() = created_by) with check (auth.uid() = created_by);

drop policy if exists "Users can delete own budget utilizations" on public.budget_utilizations;
create policy "Users can delete own budget utilizations" on public.budget_utilizations
for delete using (auth.uid() = created_by);

drop policy if exists "Super admins can manage all budget utilizations" on public.budget_utilizations;
create policy "Super admins can manage all budget utilizations" on public.budget_utilizations
for all using (
  exists (
    select 1
    from public.profiles
...
drop policy if exists "Budget Utilization PDFs allow authenticated reads" on storage.objects;
create policy "Budget Utilization PDFs allow authenticated reads"
on storage.objects for select
to authenticated
using (
  bucket_id = 'cqer-budgetutil_pdf'
);

drop policy if exists "Budget Utilization PDFs allow authenticated uploads" on storage.objects;
create policy "Budget Utilization PDFs allow authenticated uploads"
on storage.objects for insert
to authenticated
with check (
  bucket_id = 'cqer-budgetutil_pdf' AND
  (storage.foldername(name))[1] = auth.uid()::text
);

drop policy if exists "Budget Utilization PDFs owner delete" on storage.objects;
create policy "Budget Utilization PDFs owner delete"
on storage.objects for delete
to authenticated
using (
  bucket_id = 'cqer-budgetutil_pdf' AND
...
-- ============================================================
-- START COPY: Impact Assessment Module
-- ============================================================
create table if not exists public.impact_assessments (
  ...
  lead_evaluator text not null,
  date_of_assessment timestamptz,
  documents jsonb not null default '[]'::jsonb
);

create index if not exists idx_impact_assessments_created_by
on public.impact_assessments (created_by);

create index if not exists idx_impact_assessments_project_id
on public.impact_assessments (project_id);

alter table public.impact_assessments enable row level security;

drop policy if exists "Users can view own impact assessments" on public.impact_assessments;
create policy "Users can view own impact assessments" on public.impact_assessments
for select using (auth.uid() = created_by);

drop policy if exists "Users can create own impact assessments" on public.impact_assessments;
create policy "Users can create own impact assessments" on public.impact_assessments
for insert with check (auth.uid() = created_by);

drop policy if exists "Users can update own impact assessments" on public.impact_assessments;
create policy "Users can update own impact assessments" on public.impact_assessments
for update using (auth.uid() = created_by) with check (auth.uid() = created_by);

drop policy if exists "Users can delete own impact assessments" on public.impact_assessments;
create policy "Users can delete own impact assessments" on public.impact_assessments
for delete using (auth.uid() = created_by);

drop policy if exists "Super admins can manage all impact assessments" on public.impact_assessments;
create policy "Super admins can manage all impact assessments" on public.impact_assessments
for all using (
  exists (
    select 1
    from public.profiles
...
drop policy if exists "Impact Assessment PDFs allow authenticated reads" on storage.objects;
create policy "Impact Assessment PDFs allow authenticated reads"
on storage.objects for select
to authenticated
using (
  bucket_id = 'cqer-needsass_pdf'
);

drop policy if exists "Impact Assessment PDFs allow authenticated uploads" on storage.objects;
create policy "Impact Assessment PDFs allow authenticated uploads"
on storage.objects for insert
to authenticated
with check (
  bucket_id = 'cqer-needsass_pdf' AND
  (storage.foldername(name))[1] = auth.uid()::text
);

drop policy if exists "Impact Assessment PDFs owner delete" on storage.objects;
create policy "Impact Assessment PDFs owner delete"
on storage.objects for delete
to authenticated
using (
  bucket_id = 'cqer-needsass_pdf' AND
