...
-- ============================================================
-- START COPY: Internal Funding Projects Module
-- ============================================================
-- Stores managed internal funding project records coming from the
-- "Manage Internal Funding Projects" form in the projects table UI.
create table if not exists public.internal_funding_projects (
  id uuid default gen_random_uuid() primary key,
  ...
  related_curricular_offering text,
  collaborating_agencies text,
  part_of_program boolean not null default false,

  program_title text,
  program_objectives text,
  program_implementing_strategies text,
  program_locale text,
  program_type_of_clientele text,
  program_number_of_clientele text,
  program_implementers_team_role text,
  program_approved_budget_cvsu numeric(14,2) not null default 0,
  program_counterpart_fund numeric(14,2) not null default 0,
  program_total_budget numeric(14,2) not null default 0,
  program_inclusive_dates jsonb not null default '[]'::jsonb,
  program_moa_no text,
  program_team_members jsonb not null default '[]'::jsonb,

  project_title text,
  project_objectives text,
  project_implementing_strategies text,
  project_locale text,
  project_type_of_clientele text,
  project_number_of_clientele text,
  project_implementers_team_role text,
  project_approved_budget_cvsu numeric(14,2) not null default 0,
  project_counterpart_fund numeric(14,2) not null default 0,
  project_total_budget numeric(14,2) not null default 0,
  project_inclusive_dates jsonb not null default '[]'::jsonb,
  project_moa_no text,

  re_council_approved_date date,
  bor_op_approved_date date,
  inception_meeting_date date,
  beneficiaries_text text,
  beneficiaries_count integer,
  sdg_goals jsonb not null default '[]'::jsonb,
  thematic_areas jsonb not null default '[]'::jsonb,

  awards jsonb not null default '[]'::jsonb,
  award_documents jsonb not null default '[]'::jsonb,
  created_by uuid references public.profiles(id) on delete cascade not null,
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'internal_funding_projects_beneficiaries_count_check'
  ) then
    alter table public.internal_funding_projects
      add constraint internal_funding_projects_beneficiaries_count_check
      check (beneficiaries_count is null or beneficiaries_count >= 0);
  end if;
end
$$;

alter table public.internal_funding_projects enable row level security;

drop policy if exists "Users can view own internal funding projects" on public.internal_funding_projects;
create policy "Users can view own internal funding projects" on public.internal_funding_projects
  for select using (auth.uid() = created_by);

drop policy if exists "Users can create own internal funding projects" on public.internal_funding_projects;
create policy "Users can create own internal funding projects" on public.internal_funding_projects
  for insert with check (auth.uid() = created_by);

drop policy if exists "Users can update own internal funding projects" on public.internal_funding_projects;
create policy "Users can update own internal funding projects" on public.internal_funding_projects
  for update using (auth.uid() = created_by);

drop policy if exists "Users can delete own internal funding projects" on public.internal_funding_projects;
create policy "Users can delete own internal funding projects" on public.internal_funding_projects
  for delete using (auth.uid() = created_by);

create index if not exists idx_internal_funding_projects_project_id on public.internal_funding_projects (project_id);
create index if not exists idx_internal_funding_projects_created_by on public.internal_funding_projects (created_by);
create index if not exists idx_internal_funding_projects_project_no on public.internal_funding_projects (project_no);
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'internal_funding_projects'
  ) then
    alter publication supabase_realtime add table public.internal_funding_projects;
  end if;
end
$$;
-- ============================================================
-- END COPY: Internal Funding Projects Module
-- ============================================================
