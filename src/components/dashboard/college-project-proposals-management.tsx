...
interface CollegeProjectProposalsManagementProps {
...
export function CollegeProjectProposalsManagement({
  initialProjects,
  userType,
  department,
  unit,
  unitOptions = [],
  currentUserId,
}: CollegeProjectProposalsManagementProps) {
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      .channel("college-project-proposals-live")
  ...
  const departmentUnitRecords = React.useMemo(
    () =>
      filteredByEntity.filter(
        (record) => record.created_by !== currentUserId && record.created_by_user_type === "unit_coordinator"
      ),
    [filteredByEntity, currentUserId]
  );
  ...
  const scopedRecords = React.useMemo(() => {
    ...
    if (selectedScopes.includes("department_files")) addRecords(departmentRecords);
    return records;
  ...
  const toggleUnitFilter = (unitValue: string) => {
    setSelectedUnits((prev) => (prev.includes(unitValue) ? prev.filter((unit) => unit !== unitValue) : [...prev, unitValue]));
  };

  const toggleScopeFilter = (value: string) => {
    setSelectedScopes((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]));
  };
        ...
        <CardHeader className="pb-3 pt-4 px-4 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <CardDescription className="text-[10px]">Manage your project proposals and department-shared proposal records.</CardDescription>
            </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                {unitOptions.map((unitOption) => (
                  <DropdownMenuCheckboxItem key={unitOption} className="text-[10px]" checked={selectedUnits.includes(unitOption)} onCheckedChange={() => toggleUnitFilter(unitOption)}>
                    {unitOption}
                  </DropdownMenuCheckboxItem>
                ))}
              </DropdownMenuContent>
