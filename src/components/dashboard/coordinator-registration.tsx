            ...
            <div className="text-center space-y-4 py-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground px-4">
                  Temporary passwords will be autogenerated and shown in the next step.
                </p>
              </div>
            </div>
          )}
          ...
          {currentStep === 4 && (
            <div className="space-y-4">
              <div className="text-center space-y-1 pb-2">
                ...
                <p className="text-[10px] text-muted-foreground">Please copy and send these temporary credentials to the coordinators.</p>
              </div>
                ...
                {results.map((result, idx) => (
                  <div key={idx} className="p-3 rounded-lg border border-border/50 bg-muted/10 space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      {result.success ? (
                         <span className="text-[9px] px-1.5 py-0.5 rounded bg-green-100 text-green-700 font-bold uppercase">Success</span>
                      ) : (
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-bold uppercase">Error</span>
                      )}
                    </div>
                    {result.success ? (
                      <div className="flex items-center justify-between gap-2 p-2 bg-background border border-border/80 rounded">
                        <code className="text-[11px] font-mono text-[#159E44]">{result.tempPassword}</code>
                        <Button 
                          variant="ghost" 
                          size="icon" 
                          className="h-6 w-6" 
                        ...
                        >
                          {copied === result.email ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                        </Button>
                      </div>
                    ) : (
