...
type FilterMode = "all" | "with_documents" | "this_year" | "international";

interface AwardsRecognitionManagementProps {
  records: AwardsRecognitionRecord[];
  projects: Project[];
}

function getDateLabel(record: AwardsRecognitionRecord) {
  return record.date_received ? format(new Date(record.date_received), "MMM d, yyyy") : "-";
}
...
async function exportExcel(records: AwardsRecognitionRecord[]) {
  ...
  const sheet = workbook.addWorksheet("Awards Recognition");
  const columns = [
    { header: "Award / Recognition", key: "award", width: 34 },
    { header: "Donor / Awarding Body", key: "donor", width: 30 },
    { header: "Level", key: "level", width: 16 },
    { header: "Date Received", key: "date", width: 18 },
    { header: "Project", key: "project", width: 28 },
    { header: "Event / Conference", key: "event", width: 32 },
  ];
  sheet.columns = columns.map((column) => ({ key: column.key, width: column.width }));
  sheet.mergeCells("A1:F1");
  sheet.getCell("A1").value = "Awards and Recognition";
  ...
  anchor.href = url;
  anchor.download = `awards-recognition-${new Date().toISOString().slice(0, 10)}.xlsx`;
  anchor.click();
  URL.revokeObjectURL(url);
}

async function exportPdf(records: AwardsRecognitionRecord[]) {
  const { jsPDF } = await import("jspdf");
  const { default: autoTable } = await import("jspdf-autotable");
  const doc = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" });
  doc.setFontSize(12);
  doc.text("Awards and Recognition", doc.internal.pageSize.getWidth() / 2, 12, { align: "center" });
  autoTable(doc, {
    startY: 18,
    head: [[
      "Award / Recognition",
  ...
  doc.save(`awards-recognition-${new Date().toISOString().slice(0, 10)}.pdf`);
}

export function AwardsRecognitionManagement({
  records,
  projects,
}: AwardsRecognitionManagementProps) {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = React.useState("");
  const [filterMode, setFilterMode] = React.useState<FilterMode>("all");
  const [selectedRecord, setSelectedRecord] = React.useState<AwardsRecognitionRecord | null>(null);
  const [editingRecord, setEditingRecord] = React.useState<AwardsRecognitionRecord | null>(null);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [successOpen, setSuccessOpen] = React.useState(false);
  const [deleteTarget, setDeleteTarget] = React.useState<AwardsRecognitionRecord | null>(null);
  ...
  const filteredRecords = React.useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      if (filterMode === "this_year") {
        const dateYear = record.date_received ? new Date(record.date_received).getFullYear() : null;
        if (dateYear !== year) return false;
      }
      if (filterMode === "international" && record.level !== "international") return false;
      return true;
    });
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteAwardsRecognition(deleteTarget.id);
    if (result?.error) {
      alert(result.error);
      return;
    }
    ...
    <div className="space-y-5">
      <Card className="border-border/60 shadow-sm">
        <CardHeader className="space-y-4 pb-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <CardTitle className="text-xl font-semibold">Awards and Recognition</CardTitle>
              <CardDescription className="text-sm">
                Search, filter, export, and manage awards and recognition records in one place.
              </CardDescription>
            </div>
          ...
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative w-full max-w-xl">
              ...
              <Input
                ...
                placeholder="Search award, awarding body, event, project, level, or date..."
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuCheckboxItem checked={filterMode === "international"} onCheckedChange={() => setFilterMode("international")}>
                  International only
                </DropdownMenuCheckboxItem>
              </DropdownMenuContent>
            </DropdownMenu>
                  ...
                  paginatedRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="py-4 text-sm">
                        <DocumentPreview documents={record.documents} bucket="cqer-awards_pdf" />
                      </TableCell>
                      <TableCell className="py-4">
      ...
      <Dialog open={successOpen} onOpenChange={setSuccessOpen}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle>Awards and recognition saved</DialogTitle>
            <DialogDescription>
              The awards and recognition record was saved successfully and the table has been refreshed.
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle>Delete awards and recognition</DialogTitle>
            <DialogDescription>
              This will permanently remove <span className="font-medium text-foreground">{deleteTarget?.award_title}</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end gap-2">
