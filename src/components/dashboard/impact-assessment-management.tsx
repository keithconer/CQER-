...
interface ImpactAssessmentManagementProps {
  records: ImpactAssessmentRecord[];
  projects: Project[];
}
...
async function exportExcel(records: ImpactAssessmentRecord[]) {
  ...
  const sheet = workbook.addWorksheet("Impact Assessment");
  const columns = [
    { header: "Project / Training Activity", key: "activity", width: 38 },
    { header: "Proponent", key: "proponent", width: 28 },
    { header: "Lead Evaluator", key: "evaluator", width: 28 },
    { header: "Date of Assessment", key: "date", width: 18 },
    { header: "Linked Project", key: "project", width: 32 },
  ];
  sheet.columns = columns.map((column) => ({ key: column.key, width: column.width }));
  sheet.mergeCells("A1:E1");
  sheet.getCell("A1").value = "Impact Assessment";
  ...
  records.forEach((record) => {
    sheet.addRow({
      ...
      evaluator: record.lead_evaluator,
      date: getDateLabel(record),
      project: record.project_id ? record.activity_name : "-",
    });
  });
  ...
  anchor.href = url;
  anchor.download = `impact-assessment-${new Date().toISOString().slice(0, 10)}.xlsx`;
  anchor.click();
  URL.revokeObjectURL(url);
}

async function exportPdf(records: ImpactAssessmentRecord[]) {
  const { jsPDF } = await import("jspdf");
  const { default: autoTable } = await import("jspdf-autotable");
  const doc = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" });
  doc.setFontSize(12);
  doc.text("Impact Assessment", doc.internal.pageSize.getWidth() / 2, 12, { align: "center" });
  autoTable(doc, {
    startY: 18,
    head: [[
    ...
    body: records.map((record) => ([
      ...
      record.proponent,
      record.lead_evaluator,
      getDateLabel(record),
      record.project_id ? "Yes" : "No",
    ])),
    theme: "grid",
    headStyles: { fillColor: [21, 158, 68], textColor: [255, 255, 255], fontSize: 8 },
    styles: { fontSize: 7, cellPadding: 2, overflow: "linebreak" },
  });
  doc.save(`impact-assessment-${new Date().toISOString().slice(0, 10)}.pdf`);
}

export function ImpactAssessmentManagement({
  records,
  projects,
}: ImpactAssessmentManagementProps) {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = React.useState("");
  const [filterMode, setFilterMode] = React.useState<FilterMode>("all");
  const [selectedRecord, setSelectedRecord] = React.useState<ImpactAssessmentRecord | null>(null);
  const [editingRecord, setEditingRecord] = React.useState<ImpactAssessmentRecord | null>(null);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [successOpen, setSuccessOpen] = React.useState(false);
  const [deleteTarget, setDeleteTarget] = React.useState<ImpactAssessmentRecord | null>(null);
  ...
  const filteredRecords = React.useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      if (filterMode === "this_year") {
        const dateYear = record.date_of_assessment ? new Date(record.date_of_assessment).getFullYear() : null;
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteImpactAssessment(deleteTarget.id);
    if (result?.error) {
      alert(result.error);
      return;
    }
    ...
    <div className="space-y-5">
          ...
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative w-full max-w-xl">
              ...
              <Input
                ...
                placeholder="Search activity, proponent, evaluator, or assessment date..."
                  ...
                  paginatedRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="py-4 text-sm">
                        <DocumentPreview documents={record.documents} bucket="cqer-needsass_pdf" />
                      </TableCell>
      ...
      <Dialog open={successOpen} onOpenChange={setSuccessOpen}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle>Impact assessment saved</DialogTitle>
            <DialogDescription>
              The impact assessment record was saved successfully and the table has been refreshed.
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle>Delete impact assessment</DialogTitle>
            <DialogDescription>
              This will permanently remove <span className="font-medium text-foreground">{deleteTarget?.activity_name}</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end gap-2">
