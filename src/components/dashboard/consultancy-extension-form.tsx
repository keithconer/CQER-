    ...
    <Form {...form}>
      <form onSubmit={(event) => event.preventDefault()} className="flex h-full min-h-0 flex-col bg-background">
        <FullscreenFormHeader
          title={initialData?.id ? (isViewOnly ? "Consultancy Details" : "Update Consultancy") : "Create Consultancy"}
          currentStep={currentStep}
          totalSteps={stepLabels.length}
          labels={stepLabels}
          onClose={onClose}
          items={[
            { icon: BriefcaseBusiness, label: "Project Link", value: selectedProject?.title || initialData?.related_project_title || "Not part of a project", minWidthClassName: "min-w-[210px]" },
            { icon: Tag, label: "Category", value: form.watch("category") || "N/A", minWidthClassName: "min-w-[140px]" },
            { icon: Building2, label: "Status", value: form.watch("status") || "N/A", minWidthClassName: "min-w-[130px]" },
          ]}
        />
